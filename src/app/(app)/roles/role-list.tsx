...
export default function RoleList({ roles, onUpdate, onDelete }: RoleListProps) {
  ...
  // Lọc roles theo search với useMemo để optimize
  const filteredRoles = useMemo(() => 
    roles.filter((role) =>
      removeAccents(role.name).includes(removeAccents(searchQuery)) ||
      (role.description && removeAccents(role.description).includes(removeAccents(searchQuery)))
    ),
    [roles, searchQuery]
  );

  // Paginated roles với useMemo để optimize
  ...
  return (
    ...
    >
      <motion.div variants={itemVariants} className="flex justify-between items-center">
        <div className="relative max-w-md">
          <motion.div whileHover={{ scale: 1.01 }}>
            ...
            <Input
              type="text"
              placeholder="Tìm kiếm vai trò (tên, mô tả)..."
              value={searchQuery}
              onChange={(e) => {
          ...
          <table className="w-full">
            <thead className="bg-gradient-to-r from-[#F5F5DC] to-[#D2B48C] border-b border-[#D2B48C]/30">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm font-semibold text-[#6B4E31] uppercase tracking-wider w-48">
                  Mô tả
                </th>
              ...
              {paginatedRoles.length === 0 ? (
                ...
                paginatedRoles.map((role, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-[#6B4E31]">
      ...
      <motion.div variants={itemVariants} className="text-sm text-[#6B4E31]/70">
        Tổng số: <span className="font-semibold text-[#6B4E31]">{roles.length}</span> vai trò
      </motion.div>
