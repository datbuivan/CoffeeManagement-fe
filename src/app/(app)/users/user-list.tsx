...
export default function UserList({ users, onUpdate, onDelete }: UserListProps) {
  ...
  // Paginated users với useMemo để optimize
  ...
  return (
    ...
    >
      <motion.div variants={itemVariants} className="flex justify-between items-center">
        <div className="relative max-w-md">
          <motion.div whileHover={{ scale: 1.01 }}>
            ...
            <Input
              type="text"
              placeholder="Tìm kiếm nhân viên (tên, mã, vai trò)..."
              value={searchQuery}
              onChange={(e) => {
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-[#D2B48C]/10">
              {paginatedUsers.length === 0 ? (
                ...
                >
                  <td colSpan={5} className="px-6 py-12 text-center text-[#6B4E31]/50">
      ...
      <motion.div variants={itemVariants} className="text-sm text-[#6B4E31]/70">
        Tổng số: <span className="font-semibold text-[#6B4E31]">{users.length}</span> nhân viên
      </motion.div>
